...
class ICM(nn.Module):
    def __init__(self, env) -> None:
        ...
        self.inverse_model = nn.Sequential(
            nn.Linear(
                np.array(env.single_observation_space.shape).prod() * 2,
                256,
            ),
            nn.ReLU(),
        ...
        self.forward_model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(256, np.array(env.single_observation_space.shape).prod()),
        )

    def forward(self, state, new_state, action):
        inverse = self.inverse_model(torch.cat([state, new_state], dim=1))
        forward = self.forward_model(torch.cat([state, action], dim=1))
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if args.icm:
                inverse, forward = icm(
                    data.observations, data.next_observations, data.actions
                )
                inverse_loss = (1 - args.beta) * nn.CrossEntropyLoss()(
                    inverse, data.actions
                )
                forward_loss = args.beta * nn.MSELoss()(forward, data.next_observations)
                with torch.no_grad():
                    intrinsic_reward = args.alpha * (
                        (forward - data.next_observations).pow(2)
                    ).mean(dim=1)
